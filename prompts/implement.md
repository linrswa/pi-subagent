---
description: Full implementation workflow using explorer -> planner -> worker
argument-hint: "<task>"
---

Use the subagent tool with the chain parameter and `wait: true` to execute this workflow:
...
Execute this as a chain with `wait: true` and pass output between steps via `{previous}`.
